...
from qc_thesis.modeling.shared import StackSet, build_stack_frame as core_build_stack_frame
from qc_thesis.modeling.neural import score_predictions
    ...
    *,
    ...
    for _ in range(int(n_synthetic)):
        ...
        if subgroup is not None and subgroup in subgroup_cache:
            ...
            _, neighbors = subgroup_nbrs.kneighbors(X_scaled[i : i + 1], return_distance=True)
            candidates = subgroup_idx[neighbors[0]]
            candidates = candidates[candidates != i]
        else:
            _, neighbors = global_nbrs.kneighbors(X_scaled[i : i + 1], return_distance=True)
...
def _fit_anchor_scores(
    *,
    ...
    for fold_id, (train_idx, val_idx) in enumerate(_grouped_oof_splits(matched_train_df, n_splits), start=1):
        ...
        y_train = np.concatenate([np.zeros(len(train_idx), dtype=int), np.ones(len(fold_unmatched_x), dtype=int)])
    ...
    y_full = np.concatenate([np.zeros(len(matched_train_x), dtype=int), np.ones(len(unmatched_x), dtype=int)])
    clf = HistGradientBoostingClassifier(
        loss="log_loss",
        learning_rate=config.anchor_learning_rate,
        max_depth=config.anchor_max_depth,
