...
from crash_tests.ml_statistics_workbench.methods import build_stack_frame, make_metric_row, make_prediction_frame, mask_feature_columns, score_predictions
...
def _prepare_frames(config: FMissWaveformTransferConfig):
...
def _run_with_config(config: FMissWaveformTransferConfig) -> Path:
    ...
    variants = [
        ("fmiss_reference_anchor_stack_xgboost", train_frame, unlabeled_frame, test_frame),
        (
