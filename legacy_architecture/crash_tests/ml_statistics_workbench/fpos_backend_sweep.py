...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the fpos backend sweep benchmark")
...
def _fit_backend_predict(
    *,
    ...
    if backend_id == "lightgbm_default":
    ...
    elif backend_id == "xgboost_default":
    ...
    elif backend_id == "xgboost_shallow":
        from xgboost import XGBRegressor
    ...
    elif backend_id == "catboost_default":
...
def _run_with_config(config: FPosBackendSweepConfig) -> Path:
    ...
    source_train = base_row["train_frame"]
    source_unlabeled = base_row["unlabeled_frame"]
    source_test = base_row["test_frame"]
    ...
    feature_sets = [
        (
            "source_reweighted_cluster_trust_pseudo",
            source_train,
            source_unlabeled,
            source_test,
        ),
        (
            "source_reweighted_anchor_stack_cluster_trust_pseudo",
            anchor_train,
            anchor_unlabeled_frame,
            anchor_test_frame,
        ),
    ]
    backend_ids = [
    ...
    for feature_name, train_frame, unlabeled_frame, test_frame in feature_sets:
        _log(config, f"[{feature_name}] backend sweep")
        for idx, backend_id in enumerate(backend_ids, start=1):
            cv_metrics = _score_backend_cv(
            ...
            holdout_pred = _fit_backend_holdout(
                labeled_frame=train_frame,
                labeled_y=y_train,
                pseudo_selected=trust_selected,
                unlabeled_frame=unlabeled_frame,
            ...
            variant_id = f"{feature_name}_{backend_id}"
            metric_rows.append(
                make_metric_row(
                    target="fpos",
                    ...
                    source_target="fpos",
                    metrics=metrics,
                    notes=f"{feature_name} with {backend_id}",
                )
            )
